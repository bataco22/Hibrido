...
// Centro Quant C v1 · challenger prospectivo congelado sobre v6.11.7 · QRA-09 laboratorio de salidas.
// IMPORTANTE: usa namespace localStorage quantc_ y NO comparte ledger/configuración con Quant A.
(function(){
...
const ENTRY_SYNC_STARTED_AT = Number(localStorage.getItem("quantc_entry_sync_started_at")||0) || Date.now();
localStorage.setItem("quantc_entry_sync_started_at",String(ENTRY_SYNC_STARTED_AT));
...
const QRA04_STORAGE_KEY = "quantc_qra04_breadth";
const QRA04_MAX_SNAPSHOTS = 1500;
const QRA05_VERSION = "QRA05-PHASE-2026-08-25-1";
const QRA05_STARTED_AT = Number(localStorage.getItem("quantc_qra05_started_at")||0) || Date.now();
localStorage.setItem("quantc_qra05_started_at",String(QRA05_STARTED_AT));
const QRA06_VERSION = "QRA06-TIMING-CONTEXT-OOS-1";
const QRA06_STARTED_AT = Number(localStorage.getItem("quantc_qra06_started_at")||0) || Date.now();
localStorage.setItem("quantc_qra06_started_at",String(QRA06_STARTED_AT));
const QRA06_TFS = Object.freeze(["15m","1h","4h","1d"]);
const QUANT_C_VERSION = "CENTRO-QUANT-C-V1-HUMAN-MACHINE-2026-08-31";
const QUANT_C_STARTED_AT = Number(localStorage.getItem("quantc_c_started_at")||0) || Date.now();
localStorage.setItem("quantc_c_started_at",String(QUANT_C_STARTED_AT));
localStorage.setItem("quantc_c_meta",JSON.stringify({version:QUANT_C_VERSION,startedAt:QUANT_C_STARTED_AT,mode:"human-machine-experiment",historicalTrainingCutoff:"2026-08-29",control:false,challenger:true,humanIntervention:true,baseline:"Quant B v1.2 QRA09",closedTradesAtStart:0}));
const QUANT_C_MAIN_TARGET_R = 3;
const QUANT_C_LATERAL_TARGET_R = 0.25;
const QUANT_C_SHADOW_KEY = "quantc_c_shadow_trades";
...
const TRAILING_AB_STARTED_AT = Number(localStorage.getItem("quantc_trailing_ab_started_at")||0) || Date.now();
localStorage.setItem("quantc_trailing_ab_started_at",String(TRAILING_AB_STARTED_AT));
localStorage.setItem("quantc_aronson_qra_started_at",String(PROSPECTIVE_STARTED_AT));
localStorage.setItem("quantc_qra_lab_started_at",String(QRA_LAB_STARTED_AT));
localStorage.setItem("quantc_qra03_caps_started_at",String(QRA03_CAPS_STARTED_AT));
const qraRegimeCache=new Map();

const state = {
  assets: JSON.parse(localStorage.getItem("quantc_assets") || "null") || DEFAULT_ASSETS,
  weights: JSON.parse(localStorage.getItem("quantc_weights") || "null") || DEFAULT_WEIGHTS,
  market: {},
  selected: null,
  analysis: null,
  paperTrades: JSON.parse(localStorage.getItem("quantc_paper_trades") || "[]"),
  shadowTrades: JSON.parse(localStorage.getItem(QUANT_C_SHADOW_KEY) || "[]"),
  pendingPaperSignal: null,
  homeInterval: localStorage.getItem("quantc_home_timeframe") || "1d",
  autoPaper: JSON.parse(localStorage.getItem("quantc_auto_paper") || "null") || {enabled:false,interval:"4h",threshold:85,stopPct:3,targetPct:9,riskPct:1,capital:20000,lastSignals:{},universe:"top100",minQuoteVolume:5000000},
  scannerResults: []
};
...
function repairQraLabContinuity(){
  ...
  if(repaired){
    try{
      savePaperState();
      localStorage.setItem("quantc_qra_continuity_repair",JSON.stringify({version:APP_VERSION,at:Date.now(),repaired,qraLabStartedAt:QRA_LAB_STARTED_AT,qra03CapsStartedAt:QRA03_CAPS_STARTED_AT}));
...
function quantBDecision(side,qra05,qra06){
  const phase=String(qra05?.phase||"SIN_FASE").toUpperCase();
  const timing=String(qra06?.virtualDecision||"SIN_QRA06").toUpperCase();
  const base={version:QUANT_C_VERSION,decidedAt:Date.now(),phase,timing,side,action:"REJECT",branch:"NONE",targetR:null,reason:"Combinación no autorizada por Quant C v1."};
  if(timing==="CONTRA_CONTEXTO") return {...base,reason:"QRA-06 marca CONTRA_CONTEXTO."};
  if(timing==="RIESGO_ENTRADA_TARDIA") return {...base,reason:"QRA-06 marca RIESGO_ENTRADA_TARDIA."};
  // La rama lateral debe ser alcanzable: QRA-06 suele llamar OBSERVAR a una fase lateral.
  // Se permite sólo si NO está contra contexto, NO llega tarde y NO pide esperar pullback.
  if(["LATERAL","TRANSICION"].includes(phase)&&["OBSERVAR","TIMING_CANDIDATO"].includes(timing)) return {...base,action:"ENTER",branch:"B_EXP_LATERAL",targetR:QUANT_C_LATERAL_TARGET_R,reason:"Rama prospectiva lateral/transición con objetivo corto congelado 0.25R."};
  if(timing==="OBSERVAR") return {...base,action:"WAIT",reason:"QRA-06 pide OBSERVAR."};
  if(timing==="ESPERAR_PULLBACK") return {...base,action:"WAIT",reason:"QRA-06 pide ESPERAR_PULLBACK."};
  if(timing!=="TIMING_CANDIDATO") return {...base,reason:"Sin TIMING_CANDIDATO causal."};
  if(side==="long"&&phase==="REANUDACION") return {...base,action:"ENTER",branch:"C_MAIN",targetR:QUANT_C_MAIN_TARGET_R,reason:"LONG + REANUDACION + TIMING_CANDIDATO."};
  if(side==="short"&&phase==="REANUDACION") return {...base,reason:"SHORT + REANUDACION queda OFF en Quant C v1."};
  return base;
}
function compactShadowForStorage(t){
  if(t.status==="open") return t;
  const out={...t};
  if(Array.isArray(out.rPath)&&out.rPath.length){
    out.rPathSummary={rows:out.rPath.length,maxBestR:Math.max(0,...out.rPath.map(r=>Number(r?.[2])||0)),minWorstR:Math.min(0,...out.rPath.map(r=>Number(r?.[3])||0)),maxLevelR:Math.max(0,...(out.rLevelsHit||[]).map(Number))};
  }
  out.candleLog=[]; out.rPath=[]; out.candleLogCompacted=true; out.rPathCompacted=true;
  return out;
}
function saveShadowState(){
  try{localStorage.setItem(QUANT_C_SHADOW_KEY,JSON.stringify(state.shadowTrades.map(compactShadowForStorage)))}catch(e){console.warn("Quant C shadow storage",e)}
}
...
async function refreshHomeTimeframe(interval){
  ...
  localStorage.setItem("quantc_home_timeframe",interval);
  renderHome();
...
function saveAutoPaper(){localStorage.setItem("quantc_auto_paper",JSON.stringify(state.autoPaper))}
...
function saveQra04BreadthSnapshot(rows,interval,universeCount){
  ...
  try{
    ...
    localStorage.setItem("quantc_qra04_meta",JSON.stringify({version:QRA04_VERSION,mode:"research-only",startedAt:Number(localStorage.getItem("quantc_qra04_started_at")||at),snapshots:history.length,lastAt:at}));
    if(!localStorage.getItem("quantc_qra04_started_at"))localStorage.setItem("quantc_qra04_started_at",String(at));
  ...
  try{
    ...
    const scanRows=await mapWithConcurrency(universe,3,async sym=>{
      try{
        ...
        // Quant C decide ANTES de abrir. QRA-06 se reconstruye point-in-time usando sólo velas anteriores a createdAt.
        const candidate={symbol:sym,side:pick.side,interval:a.interval,createdAt,signalCandleCloseAt:signalCloseAt};
        const qra06Context=await buildQra06Context(candidate);
        const bDecision=quantBDecision(pick.side,qra05Signal,qra06Context);
        row.quantB={action:bDecision.action,branch:bDecision.branch,targetR:bDecision.targetR,phase:bDecision.phase,timing:bDecision.timing,reason:bDecision.reason};
        a.lastSignals[key]=signalId;
        if(bDecision.action!=="ENTER"){
          state.shadowTrades.unshift(makeQuantBShadow({createdAt,openedAt,sym,pick,a,analysis,signalCandle,signalCloseAt,signalAgeMs,qra05Signal,qra06Context,decision:bDecision,referenceEntry}));
          return row;
        }
    ...
    finalStatus=`${a.enabled?"Activo":"Barrido manual"} · ${completed}/${universe.length} revisadas${failed?` · ${failed} sin respuesta`:""} · ${a.interval} · score ≥ ${a.threshold}${opened?` · ${opened} nueva${opened===1?"":"s"}`:" · sin señales nuevas"}${qra04Snapshot?" · QRA-04 ✓":""} · QRA-05 fase ✓ · QRA-09 salidas ✓ · QUANT C v1 ✓`;
  }catch(e){
...
function removeAsset(sym){
  ...
  localStorage.setItem("quantc_assets",JSON.stringify(state.assets));renderAssets();fillAssetSelects();
}
...
$("#confirmAddAsset").onclick=async e=>{
  ...
  try{await getTicker(sym);state.assets.push(sym);localStorage.setItem("quantc_assets",JSON.stringify(state.assets));$("#assetDialog").close();refreshAll()}
...
function setPaperStorage(payload){ localStorage.setItem("quantc_paper_trades",payload); return true; }
...
function runStorageMaintenance(){
  ...
  try{
    ...
    localStorage.setItem("quantc_storage_maintenance",JSON.stringify({version:APP_VERSION,at:Date.now(),beforeChars:before,afterChars:payload.length,open:state.paperTrades.filter(t=>t.status==="open").length,closed:state.paperTrades.filter(t=>t.status!=="open").length,openEvidenceWindow:OPEN_EVIDENCE_WINDOW}));
...
const AUTO_CLOSE_SCAN_KEY="quantc_auto_close_scan_boundary";
...
async function activateCausalTrade(t,c){
  ...
  // Las sombras B sólo siguen el resultado contrafactual; nunca entran al ledger operativo/QRA.
  if(!t.bShadow) t.qraLab=await buildQraLabSnapshot(t.side,t.openedAt,t.interval,t.riskCash,t.capital,t.qra05Signal||null);
  if(t.qra05Signal && !t.bShadow) delete t.qra05Signal;
...
// QRA-09 no toca C_MAIN: toma la misma entrada y copia causalmente el estado
...
function renderQra09Lab(){
  ...
  if(!pool.length){
    if(stats) stats.innerHTML='<div class="notice">QRA-09 listo. Sólo contará las nuevas entradas de Quant C creadas después de activar esta versión.</div>';
  ...
  const cards=[["Cohorte QRA-09",`${pool.length} entradas compartidas · C_MAIN intacto`]];
...
async function updatePaperTrades(manual=false){
  ...
  renderPaperMonitor();
  try{
    const open=[...state.paperTrades.filter(needsPaperMonitoring),...state.shadowTrades.filter(needsPaperMonitoring)];
...
async function closePaperManual(id){
  const t=state.paperTrades.find(x=>String(x.id)===String(id));if(!t||t.status!=="open")return;
  const ok=confirm(`Cerrar ${t.symbol}/USDT ${String(t.side||"").toUpperCase()} ahora?\n\nQuant C guardará tu cierre y dejará una sombra automática para medir qué habría ocurrido sin intervención.`);
  if(!ok)return;
  const btn=document.querySelector(`[data-close-paper="${t.id}"]`); if(btn){btn.disabled=true;btn.textContent="Obteniendo precio…"}
  try{
    let exit=Number(t.current||t.entry), source="monitor_1m";
    try{
      const tk=await getTicker(t.symbol);
      const live=Number(tk?.lastPrice||tk?.price||0);
      if(live>0){exit=live;source="binance_ticker"}
    }catch(e){console.warn("ticker cierre manual",e)}
    if(!(exit>0))throw new Error("No se pudo obtener un precio válido de cierre.");

    // Contrafactual: copia exacta del trade ANTES de la intervención y continúa
    // con las reglas automáticas originales. No entra al ledger principal de C.
    const cf=JSON.parse(JSON.stringify(t));
    const now=Date.now();
    cf.id=`C-HUMAN-CF-${now}-${t.id}`;
    cf.cHumanCounterfactual=true;
    cf.humanParentId=t.id;
    cf.humanInterventionAt=now;
    cf.bShadow=true; // usa el monitor de sombras heredado; nunca cuenta como trade operativo.
    cf.bOperational=false;
    cf.notes=`QUANT C CONTRAFACTUAL · cierre humano del trade ${t.id} · sigue automático hasta su salida original`;
    state.shadowTrades.unshift(cf);

    const exitR=signedRFromPrice(t,exit);
    t.status="manual";t.exit=exit;t.closedAt=now;t.resultPct=tradeResultPct(t,exit);
    t.manualIntervention={
      version:"C-HUMAN-INTERVENTION-V1",at:now,source,exitPrice:exit,exitR,
      observedMfeR:Number(t.mfeR||0),observedMaeR:Number(t.maeR||0),
      durationMs:Math.max(0,now-Number(t.openedAt||now)),counterfactualId:cf.id
    };
    await finalizeMarketBenchmark(t);
    savePaperState();saveShadowState();renderPaperTrades();
  }catch(e){alert(`No se pudo cerrar: ${e?.message||e}`)}
  finally{const b=document.querySelector(`[data-close-paper="${t.id}"]`);if(b){b.disabled=false;b.textContent="Cerrar ahora"}}
}

function renderHumanInterventionPanel(){
  const box=$("#humanInterventionPanel"); if(!box)return;
  const open=state.paperTrades.filter(t=>t.status==="open").sort((a,b)=>Number(b.openedAt||0)-Number(a.openedAt||0));
  const manual=state.paperTrades.filter(t=>t.status==="manual"&&t.manualIntervention).sort((a,b)=>Number(b.closedAt||0)-Number(a.closedAt||0));
  const cfByParent=new Map(state.shadowTrades.filter(t=>t.cHumanCounterfactual).map(t=>[String(t.humanParentId),t]));
  const finished=manual.map(t=>({t,cf:cfByParent.get(String(t.id))})).filter(x=>x.cf&&x.cf.status!=="open"&&x.cf.exit!=null);
  const deltas=finished.map(({t,cf})=>Number(t.manualIntervention?.exitR||signedRFromPrice(t,t.exit))-signedRFromPrice(cf,cf.exit));
  const avgDelta=deltas.length?deltas.reduce((a,b)=>a+b,0)/deltas.length:0;
  const helped=deltas.filter(x=>x>0).length, hurt=deltas.filter(x=>x<0).length;
...
function tradeCard(t){
  ...
  ${t.notes?`<p class="paper-note">${t.notes.replace(/</g,"&lt;")}</p>`:""}<div class="paper-actions">${t.status==="open"?`<button class="ghost" data-close-paper="${t.id}">Cerrar ahora</button>`:"<span></span>"}<button class="danger" data-delete-paper="${t.id}">Eliminar</button></div></article>`;
}
...
function createFullBackup(){
  ...
  for(let i=0;i<localStorage.length;i++){
    ...
    if(key&&key.startsWith("quantc_")) data[key]=localStorage.getItem(key);
  }
  const backup={
    app:"Centro Quant",
    version:APP_VERSION,
...
async function restoreFullBackup(file){
  ...
  const entries=Object.entries(parsed.data).filter(([k,v])=>k.startsWith("quantc_")&&(typeof v==="string"||v===null));
  ...
  for(const [k,v] of entries){
    if(v===null) continue;
    if(["quantc_assets","quantc_weights","quantc_paper_trades","quantc_auto_paper"].includes(k)) JSON.parse(v);
  }
  ...
  const oldKeys=[];
  for(let i=0;i<localStorage.length;i++){const k=localStorage.key(i);if(k&&k.startsWith("quantc_"))oldKeys.push(k)}
  oldKeys.forEach(k=>localStorage.removeItem(k));
  for(const [k,v] of entries){
    if(v===null) continue;
    if(k==="quantc_paper_trades"){
...
$("#saveWeightsBtn").onclick=()=>{
  ...
  state.weights=next;localStorage.setItem("quantc_weights",JSON.stringify(next));alert("Pesos guardados.");refreshAll();
};
...
$("#newOosCohortBtn").onclick=()=>{
  ...
  saveAutoPaper();
  localStorage.setItem("quantc_v611_oos_cohort_started_at",String(at));
  localStorage.setItem("quantc_v611_oos_cohort_meta",JSON.stringify({version:"QRA04-OOS-v6.11",startedAt:at,mode:"prospective-clean",strategyVersion:"6.11.5"}));
